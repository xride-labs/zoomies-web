...
const marketplaceSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentListing: (state, action: PayloadAction<ListingDetails | null>) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(saveListingThunk.fulfilled, (state, action) => {
      ...
      if (listing && !state.savedListings.find((l) => l.id === action.payload)) {
    ...
    builder.addCase(unsaveListingThunk.fulfilled, (state, action) => {
      state.savedListings = state.savedListings.filter((l) => l.id !== action.payload)
    })

    // Mark as sold
    builder.addCase(markAsSoldThunk.fulfilled, (state, action) => {
      if (action.payload) {
        const listing = state.myListings.find((l) => l.id === action.payload.id)
        if (listing) {
          listing.isSold = true
        }
